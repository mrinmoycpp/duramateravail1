...
function getLatestReport(userId) {
  ...
  for (const [, report] of reports) {
    if (userId && report.userId !== userId) continue
...
const BIOMARKER_PATTERNS = [
  ...
  { name: 'Neutrophils', aliases: ['neutrophils', 'neutrophil', 'neutrophil count', 'neutrophil %', 'neutrophil percentage'], unit: '%', category: 'CBC', refMin: 40, refMax: 70 },
  ...
  { name: 'TSH', aliases: ['tsh', 'thyroid stimulating hormone', 'thyroid stim hormone'], unit: 'uIU/mL', category: 'THYROID', refMin: 0.4, refMax: 4.0 },
  { name: 'Free T3', aliases: ['ft3', 'free t3', 'triiodothyronine free'], unit: 'pg/mL', category: 'THYROID', refMin: 2.3, refMax: 4.2 },
  { name: 'Free T4', aliases: ['ft4', 'free t4', 'thyroxine free'], unit: 'ng/dL', category: 'THYROID', refMin: 0.9, refMax: 1.7 },
  ...
  { name: 'HbA1c', aliases: ['hba1c', 'glycated hemoglobin', 'haemoglobin a1c', 'a1c', 'glycosylated hemoglobin'], unit: '%', category: 'METABOLIC', refMin: 4, refMax: 5.7 },
  ...
  { name: 'eGFR', aliases: ['egfr', 'estimated gfr', 'glomerular filtration rate'], unit: 'mL/min/1.73m2', category: 'KIDNEY_FUNCTION', refMin: 90, refMax: 999 },
  ...
  { name: 'Phosphorus', aliases: ['phosphorus', 'phosphate', 'serum phosphorus'], unit: 'mg/dL', category: 'ELECTROLYTES', refMin: 2.5, refMax: 4.5 },
  { name: 'Magnesium', aliases: ['magnesium', 'serum magnesium'], unit: 'mg/dL', category: 'ELECTROLYTES', refMin: 1.7, refMax: 2.2 },
  { name: 'ALT', aliases: ['alt', 'sgpt', 'alanine aminotransferase', 'gpt'], unit: 'U/L', category: 'LIVER_FUNCTION', refMin: 7, refMax: 56 },
  { name: 'AST', aliases: ['ast', 'sgot', 'aspartate aminotransferase', 'got'], unit: 'U/L', category: 'LIVER_FUNCTION', refMin: 10, refMax: 40 },
  ...
  { name: 'Bilirubin Total', aliases: ['bilirubin total', 'total bilirubin', 's. bilirubin', 'serum bilirubin'], unit: 'mg/dL', category: 'LIVER_FUNCTION', refMin: 0.1, refMax: 1.2 },
  ...
  { name: 'Total Protein', aliases: ['total protein', 'serum total protein', 's. protein'], unit: 'g/dL', category: 'LIVER_FUNCTION', refMin: 6.0, refMax: 8.3 },
  { name: 'Albumin', aliases: ['albumin', 'serum albumin', 's. albumin'], unit: 'g/dL', category: 'LIVER_FUNCTION', refMin: 3.5, refMax: 5.0 },
  ...
  { name: 'CRP', aliases: ['crp', 'c-reactive protein', 'c reactive protein', 'hs-crp', 'high sensitivity crp'], unit: 'mg/L', category: 'INFLAMMATION', refMin: 0, refMax: 3.0 },
  ...
  { name: 'Testosterone', aliases: ['testosterone', 'total testosterone', 'serum testosterone'], unit: 'ng/dL', category: 'HORMONES', refMin: 300, refMax: 1000 },
  { name: 'Estradiol', aliases: ['estradiol', 'e2', 'oestradiol'], unit: 'pg/mL', category: 'HORMONES', refMin: 20, refMax: 250 },
...
async function parseReport(reportId) {
  ...
  try {
    ...
    const riskFlags = biomarkers.filter(b => b.status !== 'NORMAL').slice(0, 5).map(b => ({
      biomarkerName: b.rawName, value: b.parsedValue, status: b.status,
      referenceRange: `${b.appliedRefMin} - ${b.appliedRefMax} ${b.unit}`,
    }))
    const result = { fileName: `report_${reportId}.pdf`, reportDate: new Date().toISOString(), updatedAt: new Date().toISOString(), biomarkers, healthScores, riskFlags }
