...
export async function PUT(request, { params }) {
  try {
    ...
    reports.set(reportId, { reportId, fileName: file.name, status: 'UPLOADING', createdAt: Date.now() })

    let text = ''
    try {
      text = await parseBuffer(buffer)
    } catch (parseErr) {
    ...
    reports.set(reportId, { reportId, fileName: file.name, status: 'COMPLETED', createdAt: Date.now() })
    ...
    return NextResponse.json({ success: true, reportId, result })
  } catch (error) {
